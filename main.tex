...
A practical difficulty arises from the sampling mechanism required to verify Monte Carlo estimates. Each sampled password must be stored together with its probability, sorted, and processed to construct the estimation curve. In frameworks such as MAYA, this procedure is implemented using Python data structures, which incur substantial object overhead and memory fragmentation. As a result, sampling large sets—on the order of hundreds of millions of items—quickly becomes infeasible on standard hardware.
...
We could use optimized versions like John The Ripper\footnote{\url{https://www.openwall.com/john/}}, which speed up the brute forcing job but are still very slow compared to these models.
...
The Rank of a password $\alpha$ is defined as such: 
\begin{equation}
  R_\f(\alpha) = |\{\beta \mid \beta,p_\beta \in \Gamma \wedge p_\beta > p(\alpha) \wedge f(\beta) \}|
...
Monte Carlo's estimation is useful in this field \cite{ref:dellamico} because it enables us the confidence of estimating the rank of a password, and by extension how good a model is how does it work? 
...
Further more the larger the size of $\Theta$ the better the estimation as proved in \cite{ref:dellamico}, this will help us especially with Low-probability passwords since they have higher variance. 
...
To this day we have not found a way to better this variance, the latest attempts have yielded some results in lowering the variance therefore making Monte Carlo more accurate especially for Low-probability passwords, but they are not good enough yet.
...
This is a good estimation but if we need to estimate a lot of passwords, this will not be fast since each time we need to sample for Monte Carlo and calculate the rank, so the solution is to precalculate the rank and samples so that we do not need to do it multiple times,
...
\subsection{Stanek}
In Stanek's paper~\cite{ref:stanek}, the authors improved the Monte Carlo estimation of password ranks by employing different methods. They first addressed the precision problem with two ideas:
\subsubsection{Precision Problem}
\begin{enumerate}
  \item \textbf{Linear Interpolation:} Passwords are taken in rank intervals and linear interpolation is applied\footnote{Linear interpolation is a common technique used to lower the variance of a Monte Carlo estimation: https://www.math.hkust.edu.hk/~machas/scientific-computing.pdf}. This yields results with lower variance.
  \item \textbf{Uniqueness post-sampling:} After sampling, the process eliminates repeated probabilities, substituting them with a frequency count. This improves the estimation by removing redundancy from the pool, thereby strengthening the sampling. Intuitively, since Monte Carlo relies on averaging, treating duplicate values as frequencies ensures that the calculation remains efficient while making the resulting average more precise.
\end{enumerate}
However these improvements are not that substantial and they also require more time to implement so in this thesis we choose to implement the version found in ~\cite{ref:dellamico}.
\subsubsection{Speed Problem}
They decided to partition password probabilities into logarithmic buckets. This allows for search speeds faster than a binary search, which is particularly beneficial for very large samples. However, we did not implement this method in this thesis, as the marginal time savings did not justify the implementation complexity and it would have added.
...
\subsection{MAYA}
MAYA's framework introduces a standardized and extensible framework to test models, this is a strong foundation which has enabled the field to 
...
The main problem came from the fact that each item was saved as a tuple of str, float, which was appended into a list, and that list was manipulated with the heapq\footnote{\url{https://docs.python.org/3/library/heapq.html}} module.
...
\begin{itemize}
  \item \textbf{cdef}: Defines a C-only function. These are very fast but cannot be called from external Python code. By default, they act like standard Python functions and hold the GIL\footnote{The Global Interpreter Lock is a Python feature which was introduced to eliminate race conditions in parallel operations on objects, Python needs it because objects are tracked by the Garbage collector by keeping count of the references of said object when the references reach 0 then the garbage collector frees memory, if there are parallel operations on the same object this could lead to an early free inducing a use after free, or race conditions, we resolve this by using the GIL which makes each operation on objects one thread only thus preventing the majority of problems from arising. \url{https://wiki.python.org/moin/GlobalInterpreterLock}}, allowing access to Python objects. However, if defined with \texttt{nogil} (e.g., \texttt{cdef void func() nogil:}), they release the GIL; in this mode, Python structures cannot be accessed.
...
Therefore we opted to use cibuildwheel\footnote{\url{https://github.com/pypa/cibuildwheel}} which when integrated with Github's  CI/CD this facilitates software distribution.

\subsection{Heap-item}
\label{heapitem}
The first proposed solution was to build a heap-item\footnote{\url{https://github.com/BobTheBot988/heap-item}} custom module written in Cython thus maximizing performance and compatibility with Python.
A significant limitation of this approach is that, similar to the pure Python implementation, each heap item remains a distinct object managed by the garbage collector., each heap item is an object that must be tracked by python's garbage collector\footnote{\url{https://docs.python.org/3/library/gc.html}}, nevertheless this approach helped us reduce memory consumption by $\sim59.79\%$ instead of the pure python approach as seen in Figure \ref{fig:1}, however further optimization was necessary, so we opted for a custom heap called heapcy\footnote{\url{https://github.com/BobTheBot988/heapcy/tree/main}}.

Using Cython allows us to replace Python objects with C-structured
objects giving us improved memory usage while also giving us ease of integration in Python, we could have used C++ and then have written the integrations with nanobind\footnote{\url{https://nanobind.readthedocs.io/en/latest/}}, but that would have taken considerably more time and the fast software distribution would have been more complicated.
...
\subsection{IID sampling problems and solution}
 The sampling used in the MAYA implementation was not IID because it had a threshold for the low probability passwords,
...
\label{chap:5}
\section{Dataset}
For this paper we chose the RockYou\footnote{\url{https://drive.google.com/file/d/1XEsAf99H3DmH4ichbH-4yXkb0mSwoADY/view?usp=sharing}} dataset the same as in MAYA \cite{ref:maya}(split 80;20), for time reasons we couldn't test with other datasets.
...
We employ a Monte Carlo estimate \eqref{eq:rhat} to get a result similar to  \eqref{eq:realrank}.
...
As seen in Figure~\ref{fig:1}, the optimizations are substantial. These gains in memory usage will let us generate larger top-k samples, allowing us to evaluate Monte Carlo Precision for even larger numbers. Furthermore, these benefits are not limited to \textbf{FLA} and are easily extended to any model using the same generation schema.
\begin{figure}[h!]
     \centering
...
From the results shown in Table~\ref{tab:2}, we found that Monte Carlo, when used for guess number estimation, continues to be highly precise after $10^7$ ~\cite{ref:fla} even with $\Theta$ sizes as low as $10^3$. This may signify that it could be used for any model as a substitute for deterministic evaluation, rendering the process much faster and more efficient. However, the problem of single password rank estimation remains~\cite{ref:stanek}.\begin{figure}[h!tbp]
