...
Therefore we developed Password-guessing Models of our own to study and to evaluate the strength of a passoword.
...
\pagebreak
\section{Cython}
Cython ( not to be confused with CPython ) is a compiled programming language that is used to write c/c++ extensions for python very easy.
The following explains the typical workflow: 
  We first define preprocessing options that are read by the compiler with \# like in c 
  this will tell the compiler how to translate the code in c/c++ and then how to write the bindings,
  we then start writing the code in a python style syntax.
  \subsection{Cython functions}
    Cython has 3 types of functions declarators: 
    \begin{itemize}
      \item cdef this will tell the compiler to write a c-only equivalent, we cannot use any python structures, functions inside of these only code which can be directly translated to c we can turn off the \href{https://wiki.python.org/moin/GlobalInterpreterLock}{GIL} for all of the function at the definiton e.g.(cdef void my\_func() nogil:).
      \item cpdef this will be used to call c functions and is available to call in python, we can also use python functions and libraries inside of these we can also turn off the GIL with the python keyword WITH.
      \item def this are standard python functions which cannot access cdef functions directly.
    \end{itemize}
  This will allow us to write performance heavy loops in cdef functions and we can call them in cpdef functions to maximize the code simplicity.
...
\chapter{Conclusion}
summary
key result (MC reliable up to $5.0\cdot10^8$)
implication
future work
\label{chap:7}
