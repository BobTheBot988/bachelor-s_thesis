...
\lettrine[lines=2, findent=3pt, nindent=0pt]{I}{}n the field of probabilistic password-guessing models, evaluating a password's resistance to guessing attacks requires estimating its \textit{guess number}. The guess number represents how many attempts an attacker would need, on average, before generating or matching a given password. 

Computing this quantity exactly is infeasible for modern probabilistic models, as the password space is extremely large. For this reason, Monte Carlo estimation is employed to obtain an approximate rank without enumerating the full distribution. However, Monte Carlo estimation places strong computational requirements on the sampling procedure, which often becomes a bottleneck in practice.

The MAYA framework builds upon Monte Carlo methods to evaluate password-guessing models efficiently. Monte Carlo techniques, especially those derived from Dell'Amico and Filippone~\cite{ref:dellamico}, offer effective rank estimation but require very large sample sizes to achieve stable accuracy. This, in turn, demands highly efficient sampling and memory management.

During the development of this thesis, the primary challenge was to optimize MAYA’s sampling system used in the Fast, Lean and Accurate (FLA) password model. The original implementation relied heavily on Python data structures, which resulted in excessive memory usage due to object overhead. Because sampling dominates the cost of Monte Carlo estimation, reducing memory consumption and execution time became essential for enabling larger sample sizes and faster evaluations.

This thesis presents several contributions aimed at improving the efficiency of password rank estimation:
...
However, Monte Carlo estimation imposes significant computational demands. Accurate estimation requires very large sample sizes, often on the order of millions or tens of millions of generated passwords. Since each sample requires updating heap structures, memory footprint and sampling speed become the primary bottlenecks in frameworks such as MAYA.
...
The motivation behind this thesis is to extend the scalability of Monte Carlo estimation for password-guessing models by optimizing the underlying sampling mechanism. By designing more memory-efficient heap structures and reducing Python overhead through Cython, we enable substantially larger sample sizes and faster evaluation, both of which improve the accuracy and practicality of password-strength modeling.\chapter{Background and Related Work}
\label{chap:3} 
...
\begin{itemize}
  \item \textbf{cdef}: Defines a C-only function. These are very fast but cannot be called from external Python code. By default, they act like standard Python functions and hold the \href{https://wiki.python.org/moin/GlobalInterpreterLock}{GIL}\footnote{The Global Interpreter Lock is a Python feature which was introduced to eliminate race conditions in parallel operations on objects, Python needs it because objects are tracked by the Garbage collector by keeping count of the references of said object when the references reach 0 then the garbage collector frees memory, if there are parallel operations on the same object this could lead to an early free inducing a use after free, or race conditions}, allowing access to Python objects. However, if defined with \texttt{nogil} (e.g., \texttt{cdef void func() nogil:}), they release the GIL; in this mode, Python structures cannot be accessed.
...
So given a password $\alpha$ to find its rank given A and C, we need to reverse binary search  in A so that we find the index $i$ of the probability which is higher than $p(\alpha)$,
...
\begin{algorithm}[H]
    ...
    % -------------------------------------------------------
    \underline{function LoadGuessNumbers} $(path)$\;
    \Input{Path to Monte Carlo CSV results}
    \Output{Sorted list of guess numbers $G$}
    
    \tcp{1. Parse rank estimates from file}
    \vspace{0.3em}
    $RawRows \leftarrow$ Read CSV at $path$\;
    $G \leftarrow$ Extract column \texttt{rank\_hat} from $RawRows$\;
    
    \tcp{2. Sort ascending for binary search (CDF)}
    \vspace{0.3em}
    $G \leftarrow \text{SortAscending}(G)$\;
    \Return $G$\;
    \BlankLine
    \hrule
    \BlankLine
...
\begin{figure}[h!]
    ...
    \caption{Convergence of the Monte Carlo rank estimation as the sample size $|\Theta|$ increases. Subfigures (a) through (d) illustrate that at lower sample sizes ($10^1 - 10^3$), the estimation exhibits high variance (noise). As $|\Theta|$ approaches $10^5$ and $10^6$ (f), the curve stabilizes, confirming that the memory optimizations allow for sample sizes large enough to produce deterministic-like accuracy.}
    \label{fig:3}
\end{figure}
\FloatBarrier
\pagebreak
...
$5.0 \cdot 10^8$ & $59.84\%$ &$61.87\%$ & $60.45\%$ & $60.65\%$ & $60.35\%$ & $60.42\%$ & $60.47\%$ \\
 \bottomrule
\end{tabular}
\caption{Stability of the Guessing Percentage ($G(x)$) across varying Monte Carlo sample sizes. The data indicates that while estimates for high-probability passwords stabilize quickly (at $|\Theta|=10^4$), accurate estimation for lower-probability tails (higher Guess Numbers) requires the massive sample sizes ($|\Theta| \geq 10^6$).}
\label{tab:2}
